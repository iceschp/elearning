...
class MathViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "Mathcell")! as! MathTableViewCell
        let imgURL = URL(string: myCourses[indexPath.row].img!)
        let data = try? Data(contentsOf: imgURL!)
        
        cell.lbMathtable.text = myCourses[indexPath.row].title
        cell.lbMathMentor.text = myCourses[indexPath.row].mentor
        cell.lbMathRate.text = myCourses[indexPath.row].rate
        cell.lbMathImg.image = UIImage(data: data!)
        cell.lbMathImg.contentMode = .scaleAspectFit
        return cell
    }
...
//      }
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        MathTableView.separatorStyle = .none
//      tableone.showsVerticalScrollIndicator = false
//      tableone.backgroundColor = UIColor.init(red: 35, green: 40, blue: 45, alpha: 1)
       
        self.MathTableView.rowHeight = 200
        
        ref = Database.database().reference().child("coures/math")
...
//            }
    ...
    extension UIView {
        ...
        func MathTableView() -> UITableView? {
        ...
        func MathTableViewCell() -> UITableViewCell? {
        ...
        func MathTableviewcellpath() -> IndexPath? {
